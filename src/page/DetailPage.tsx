...
import { ArrowLeft, Star, Heart, ChevronDown, Share2 } from "lucide-react";
import {
  tmdbService,
  addFavorite,
  addMovieRating,
...
type Action =
  ...
  | {
      type: "SET_USER_DATA";
      payload: { userRating: number | null; isFavorite: boolean };
    }
  ...
  | {
      type: "ADD_EPISODES";
      payload: { seasonNumber: number; episodes: any[] };
    };
...
const reducer = (state: DetailState, action: Action): DetailState => {
  switch (action.type) {
    ...
    case "ADD_EPISODES":
      return {
        ...state,
        episodesMap: new Map(state.episodesMap).set(
...
const DetailPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        dispatch({
          type: "SET_ERROR",
          payload: "Failed to load details. Please try again.",
        });
      } finally {
    ...
    fetchData();
  }, [type, id, user, sessionId]); // Menambahkan dependencies yang hilang: user, sessionId
  ...
  const toggleSeason = async (seasonNumber: number) => {
    ...
    if (!state.episodesMap.has(seasonNumber)) {
      try {
        ...
        dispatch({
          type: "ADD_EPISODES",
          payload: { seasonNumber, episodes: seasonData.episodes || [] },
        });
      } catch (err) {
  ...
  const handleShare = async () => {
    const shareData = {
      title: title,
      text: `Check out this ${type === 'movie' ? 'movie' : 'TV show'}: ${title}`,
      ...
      <div className="min-h-screen bg-black text-white flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400 mb-6">
            {state.error || t("detail.itemNotFound")}
          </p>
          <Link to="/">
  ...
  const title =
    "title" in state.details ? state.details.title : state.details.name;
  const releaseDate =
    "release_date" in state.details
      ? state.details.release_date
      : state.details.first_air_date;
  const runtime = "runtime" in state.details ? state.details.runtime : null;
  const numberOfSeasons =
    "number_of_seasons" in state.details
      ? state.details.number_of_seasons
      : null;
  const numberOfEpisodes =
    "number_of_episodes" in state.details
      ? state.details.number_of_episodes
      : null;
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <div className="relative">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent" />
        <div className="absolute left-4 md:left-20 bottom-4">
          <Link to="/">
            <Button
              variant="outline"
              size="sm"
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <div className="h-full flex flex-col items-center md:items-start">
            ...
            {type === "tv" &&
              ...
              state.details.seasons.length > 0 && (
                <div className="mb-8 max-w-80 h-full mt-10 ">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-4 mb-2">
              ...
              <Button onClick={handleShare} variant="outline" size="sm" className="text-white border-white">
        ...
        {state.similar.length > 0 && (
          <div className="mb-8">
            <h2 className="text-2xl font-semibold mb-4">
              {type === "movie"
                ? t("detail.similarMovies")
                : t("detail.similarTVShows")}
            </h2>
