...
// --- CLASS UTAMA: TMDB SERVICE ---

class TMDbService {
  private apiKey: string;
  private currentLanguage: string = "en-US"; // Additional: state for dynamic language
  ...
  // Helper: Pemetaan Path Gambar menjadi URL lengkap
  private mapImagePaths(results: any[]): any[] {
    const IMAGE_SIZE_POSTER = "w500";
    const IMAGE_SIZE_BACKDROP = "original";
    const IMAGE_SIZE_PROFILE = "w500";

    return results.map((item) => {
      // Gabungkan Base URL, Ukuran, dan Path.
      if (item.poster_path)
...
// --- EKSPOR INSTANCE DAN FUNGSI CONVENIENCE ---
